...
| VPN-side DNS | no DNS server advertised by current gateway; school profile supplies two internal resolvers | DNS codec, exact txid/question, TC, partial TCP, oversize, timeout, source merge, policy, race and cache fixtures | gateway + reviewed profile sources through userspace UDP; matching TC retries the same resolver over userspace TCP | HPC hostname and truncated-answer canary required per release |
