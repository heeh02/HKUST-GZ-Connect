...
- Status: Current product contract; does not authorize unverified protocol capability
- Owner: product and architecture maintainers
- Last verified: 2026-09-04
- Current implementation baseline: stable tag `v2.0.0` at
  `415a3b5dfb9d11bffa14cde27e924af486823249`
- Implementation status: [`../2.0-status.md`](../2.0-status.md); P1-P8 and the 2.0 UI
  convergence are released, while real Gateway MFA/P9 is not implemented
- Packaged 2.0.0: HKUST(GZ) is the only reviewed Profile, while `Other school` provides an explicitly
  unverified Custom Gateway path; formal reviewed multi-school support still requires a second reviewed Profile

HKUST(GZ) is the only reviewed Profile in 2.0.0. Other schools remain user-selectable through
a credential-free probe, exact-origin confirmation, isolated Custom Profile/Account/Workspace and persistent
unverified status. This path reports only capabilities the selected Gateway actually proves; it never promotes
an arbitrary Gateway to reviewed or fully compatible status.
...
Integration Center provides profile-bound Clash/Mihomo copy/save exports and a copy-only VS Code Remote-SSH
snippet. It never installs or modifies third-party software. Those integrations are not campus resources or
ordinary-user launchers.
...
|   |     redacted preview / copy / single-file YAML export
|   `-- Connection Center
...
- an app-owned local Workspace Home that shows the active school, trust state, favorites, recent sites and
  remaining campus services without making a network request;
...
### Implemented 2.0 boundary

Campus Workspace preserves the existing Browser behavior and now adds:
...
- a local new-tab/Home resource surface with no remote school-home fallback;
...
- one copy/save-only Clash/Mihomo-compatible YAML plus a copy-only VS Code Remote-SSH configuration snippet;
...
Every action is Profile/Account-bound and uses a redacted preview. Saved exports use owner-only atomic output
and readback validation. The product does not provide an integrated
...
- one generic copy/save adapter for Clash/Mihomo-compatible YAML;
- a copy-only VS Code Remote-SSH host template using the packaged `ProxyCommand` helper;
- no third-party download, installation, discovery, launch, subscription mutation or SSH config editing;
- redacted preview followed only by explicit copy or save;
...
decision backed by a real need. The complete adapter and atomic export contract is
...
| Proxy tools | hidden | copy/save Clash or Mihomo configuration and copy VS Code snippet | configured loopback listener |
| Custom Gateway | explicit unverified school onboarding | probe evidence, capability detail and safe deletion | unsupported for unattended Headless use |
...
- HKUST(GZ) as the sole reviewed packaged Profile;
- an `Other school` path with credential-free probe, exact-origin confirmation, isolated state, persistent
  unverified presentation and tombstone-safe deletion;
...
- an External Tool Integration Center for one profile-bound Clash/Mihomo YAML and a VS Code Remote-SSH
  configuration snippet;
- transactional redacted preview, explicit copy/save, owner-only single-file output and parse/readback validation;
...
- any claim that a Custom Gateway supports password authentication, L3, DNS, MFA or resources before its own
  bounded runtime evidence proves that capability;
...
| External tool integration | This export is stale, incompatible or could not be written safely | Regenerate for the active profile/account or choose a new output file | disable proxy authentication, overwrite subscriptions or reuse another profile's credential |
...
- Renderer, argv, logs and telemetry contain generated proxy credentials or full configuration exactly `0`
  times;
- failed copy/save preparation or export write/readback transactions leave current integration state ambiguous
  exactly `0` times;
...
| Current Web shortcut model | [`campus-resources.js`](../../desktop/lib/resources/runtime/campus-resources.js), [`campus-resource-store.js`](../../desktop/lib/browser/resources/campus-resource-store.js), [`builtin-resources.json`](../../desktop/assets/profiles/hkustgz/builtin-resources.json) | bounded Web-only resource fields, routes and local editing |
| Current Browser surface | [`campus-browser.html`](../../desktop/renderer/campus-browser.html), [`campus-browser.js`](../../desktop/lib/browser/session/campus-browser.js) | tabs, toolbar, route control, download, error and credential behavior |
| Browser Session and request gate | [`browser-session-manager.js`](../../desktop/lib/browser/session/browser-session-manager.js) | one persistent PAC Session, transaction and fail-closed gate |
| Resource-triggered connection limitation | [`campus-browser-manager.js`](../../desktop/lib/browser/session/campus-browser-manager.js), [`campus-open-policy.js`](../../desktop/lib/browser/resources/campus-open-policy.js) | manager currently connects before open; route helper can distinguish Direct |
| Current typed errors | [`engine-output.js`](../../desktop/lib/connection/engine/engine-output.js), [`login-flow.js`](../../desktop/lib/browser/auth/login-flow.js) | existing stable categories and current UI progression |
...
| External tool integration boundary | [`ADR-0005`](../adr/0005-external-tool-integration-center.md), [`generic-export-adapters.js`](../../desktop/lib/integrations/generic-export-adapters.js), [`external-proxy-config.js`](../../desktop/lib/integrations/external-proxy-config.js) | profile-bound copy/save export and VS Code snippet generation; never third-party installation or mutation |
| Historical 2.0 network vision | [`2.0 Vision`](../archive/2.0/2.0-vision.md) | Original Ingress, Router, Exit and Underlay design rationale |
...
This evidence map records the 2.0 design contract. Current support claims come from
[`../2.0-status.md`](../2.0-status.md); any remaining target still requires matching implementation,
tests, package evidence and—where proprietary behavior is involved—an authorized school canary.
