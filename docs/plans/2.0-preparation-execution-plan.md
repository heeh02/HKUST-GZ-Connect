...
高级用户
  → 选择 Clash YAML / Mihomo YAML / VS Code Remote-SSH 配置片段
...
closed IntegrationAdapterRegistry + profile/account-bound transaction
        ↓
Clash YAML / Mihomo YAML / VS Code Remote-SSH snippet
...
- provide only Clash YAML and Mihomo YAML copy/save plus a copy-only VS Code Remote-SSH `ProxyCommand` template;
