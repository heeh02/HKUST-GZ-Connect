...
- Status: Historical implementation ledger; it does not redefine the approved Revision 4 phase numbering
- Revision: 11 — records product correction, safe packaged Other-school onboarding, and paused P9
...
高级用户
  → 选择 Clash/Mihomo YAML / VS Code Remote-SSH 配置片段
  → 预览当前profile/account绑定与认证要求
  → 显式复制/保存；不安装、发现、修改或启动第三方工具
```

第一个打包Beta把HKUST(GZ)作为唯一reviewed Profile，同时显示明确未审核的`Other school`入口。
Custom Gateway必须经过安全connector、隔离存储、无凭据probe、一次性精确origin确认和完整删除
生命周期；第二个reviewed school仍是宣告正式reviewed multi-school支持的前置门。
...
三项目逐功能映射见
[`2.0-cross-project-feature-map.md`](../archive/2.0/2.0-cross-project-feature-map.md)，多学校安全决策见
...
P6 custom Gateway profiles are Advanced/experimental, local and minimal: standard PKI, no private DNS
...
closed IntegrationAdapterRegistry + profile/account-bound transaction
        ↓
Clash/Mihomo YAML / VS Code Remote-SSH snippet

Advanced custom Gateway onboarding (implemented P6 experimental path)
        ↓
...
Profile Network Rules
  ...
  └── VS Code/Profile-bound ProxyCommand generator
...
semantically equivalent before activation. Clash/Mihomo rules and VS Code ProxyCommand profile binding are
...
4. **Per-ingress policy** — Campus Browser may evaluate Campus/Direct rules. Existing SOCKS/HTTP and every
   ...
   Clash/Mihomo/VS Code/SOCKS traffic Direct.
...
transactional preview/export and no-unscoped-write contract is fixed by
[`ADR-0005`](../adr/0005-external-tool-integration-center.md).
...
| --- | --- | --- |
| Multi-school profiles | Implemented reviewed HKUST anchor + isolated experimental `Other` | A second independently reviewed school remains the gate for a formal multi-school support claim |
| Account/workspace scope | Implemented P3/P4 | Preserve Profile/Account/Workspace storage, Browser partition and event-epoch isolation |
| Safe Gateway connection | Implemented P5 generation owner | Reviewed HKUST HTTP/special-TLS sockets share the connector; P6 custom onboarding reuses public-address validation and one-shot confirmation |
| Custom Gateway onboarding | Implemented `I3/E3` experimental path | Probe reports only public auth surface while L3/DNS/transport remain unknown until authenticated runtime evidence |
...
| Campus Workspace | Implemented P8 | Keep it account-workspace-scoped, resource-first and the principal ordinary-user interface |
| Web resource search/launcher | Implemented P8 | Reviewed/local Web only with category/search/favorite/recent and user-removable built-ins |
| External Tool Integration Center | Implemented and narrowed after product test | Clash/Mihomo copy/save plus copy-only VS Code snippet; no third-party install, discovery or config mutation |
| Second reviewed school | None | Required before a formal reviewed multi-school support claim; not treated as a retroactive prerequisite for implemented P7/P8-local architecture work |
...
| Profile Network Rules | Implemented P7 | Browser, internal PAC and external export generators consume one Profile/account rule source |
...
- route settings, credential presence, explicit connection credential ownership and Account login state through
  one immutable-mode persistence adapter, requiring a process relaunch instead of dual authority when migration
...
### 2.0-P6 — Implemented school selector and isolated experimental custom domain

- add the school selector while keeping one active Profile/Account/Engine;
- expose `Other` only as a clearly experimental/unverified custom-local Profile through the P5 safe connector,
  credential-free probe and one-use confirmation;
- prove storage, Browser, credential, route and event isolation with synthetic/adversarial tests and 100 switches;
- retain a second independently maintained reviewed Profile, official parity and canary as the gate for a formal
  multi-school production-support claim;
- the P6 custom path accepts only a bounded HTTPS domain/port, validates CNAME/A/AAAA and connected peer, uses standard PKI and
...
### 2.0-P7 — Shared Profile Network Rules and External Tool Integration Center (implemented)

- define one Profile/Account rule source for campus domains, direct domains, campus CIDRs, Gateway bypass and
  profile identity;
- make Campus Browser evaluation, internal PAC generation, Clash/Mihomo rules and VS Code Profile binding consume
  that source without introducing a Router process;
- provide one Clash/Mihomo-compatible YAML copy/save plus a copy-only VS Code Remote-SSH `ProxyCommand` template;
- preview exact bounded output before confirmation; save only to the file explicitly selected by the user using
  owner-only atomic apply and readback validation;
- expose no install/update/remove action and retain no third-party target ownership record;
...
- keep Campus proxying only in the packaged `ProxyCommand`; users own `Host`, `HostName`, `User` and remote `Port`;
- do not scan disks/processes, overwrite subscriptions/config files, launch/install third-party tools or weaken
...
- integration export binds exact profile/account/listener/auth/policy revisions; old
  credentials fail after switch;
- generated tool credentials/config remain Main-owned until explicit copy/save and never
  enter Renderer, argv, logs or telemetry;
- copy/save fault injection performs no unconfirmed clipboard/file mutation; save is all-old/all-new or explicitly
  rollback-incomplete;
- adapters never scan, install, discover, launch or modify third-party software; only a user-selected YAML export
  file can be written;
...
P2-P5 are implemented foundation. The custom-domain path is development-only and is not a packaged-beta
feature. The implementation labels formerly called P6-P8 are retained only as history and do not replace the
approved Revision 4 sequence. Product correction precedes any P9 work; P9 requires real school evidence. P10-P12+
