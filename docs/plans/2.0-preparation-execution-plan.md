...
- Revision: 7 — records implemented P1-P7 and keeps P8/P9 independently gated
...
学校选择器以reviewed Profiles为可信锚点，并提供明确的`Other`实验路径。该路径只在P3-P6的
安全connector、隔离存储、无凭据probe和一次性确认边界后创建custom-local Profile；第二个
reviewed school仍是正式多学校支持的证据门，而不是该实验路径的追溯前置条件。
...
| External Tool Integration Center | Implemented P7 | Generic export, Clash Verge Rev owned-extension lifecycle and OpenSSH managed Include/profile conf with credential rotation, preview/backup/atomic validate/rollback; no scan/unowned overwrite/launch |
