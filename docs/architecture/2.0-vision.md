...
Clash Verge Rev、OpenSSH、VS Code、PAC或手工客户端；EasyConnect/aTrust provider、Routing、
Exit与Underlay属于底层运行时。

HKUST(GZ)是首个reviewed profile，而不是产品边界。当前学校选择器允许通过安全connector和
隔离存储创建实验性、明确未验证的`Other` Gateway；第二个reviewed school仍是正式多学校支持
声明的证据门，而不是已实现experimental path的追溯前置条件。每个profile/account
...
进入新学校。Clash Verge Rev只修改owned extension；OpenSSH只管理一次性`Include`与profile conf；
其他client必须由用户选择exact target且只修改owned block。所有写入先显示diff，执行backup、
atomic replace、parse/readback与rollback；不扫描磁盘/进程、不覆盖subscription/unowned block、
不安装VS Code extension、不启动terminal/app。OpenSSH `Port`始终是remote port，Campus代理只用
`ProxyCommand`。
