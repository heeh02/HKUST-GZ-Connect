...
- HKUST(GZ) Connect当前已提交基线固定为
  `de35d652fe5cb4681830782a2929d20f4eca9df3`；未提交的`codex/2.0-product-convergence`
  worktree由[`2026-08-26 Phase 0审计`](../audits/2026-08-26-2.0-product-convergence-baseline.md)
  单独约束；已发布v1.2.3仍固定为`5d8323d`。post-release convergence只提升生命周期实现/测试
  证据，不提升Gateway capability等级；
...
| Safe Gateway connector | 内部Agent/privileged服务；本次仅静态线索 | underlay支持显式/自动接口绑定；无同等自定义域名SSRF边界 | `I3/E3`；HTTP/Modern socket同generation并在凭据前验证 | 当前HKUST exact-SHA E4与第二学校证据仍独立补充 |
...
| SOCKS5 TCP | N/A：非官方主要接口 | Source production | `I3/E3`；exact-SHA三平台package已通过，真实Clash E4待补 | local p95 ≤10 ms；固定基线回退≤5% |
...
| External Tool Integration Center | N/A：官方native integration不适合作为独立客户端模板 | CLI/local proxy配置，secret与profile边界不同 | `I3/E3` convergence worktree；仅Clash/Mihomo copy或用户选址单文件export，以及copy-only VS Code片段；无安装、发现、managed block或第三方配置修改 | 真实第三方客户端导入/连接仍是独立canary，不由文件写入测试代替 |
...
| CI evidence | 不公开 | 固定审计build强、test gate弱 | `I3/E3` remote CI与cross-platform package绿色；`main` required governance待补 | every PR 三平台 required checks |
