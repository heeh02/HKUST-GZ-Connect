...
| External tool integration | Portal/Agent owns native integrations; static presence is not a safe template | Local proxy is configured through CLI/user config | Main-generated Clash-compatible YAML, PAC and OpenSSH ProxyCommand over an authenticated loopback frontend | Profile/account-bound Center: generic export, Clash Verge Rev owned extension, OpenSSH managed Include/profile conf and user-selected managed blocks; diff/backup/atomic validate/rollback, no scan/subscription overwrite/launch |
