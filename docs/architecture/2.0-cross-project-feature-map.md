...
| External tool integration | Portal/Agent owns native integrations; static presence is not a safe template | Local proxy is configured through CLI/user config | P7 Profile/account-bound Center with generic export, Clash Verge Rev and OpenSSH managed lifecycle | Preserve no-scan/no-subscription-overwrite/no-launch boundary; real client use remains canary evidence |
