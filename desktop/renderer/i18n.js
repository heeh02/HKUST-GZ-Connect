...
(function (root, factory) {
  ...
  const dictionaries = {
    zh: {
      ...
      'tower.strictProxyAuthSummary': '默认关闭即可，不影响校园浏览器、Clash 或 SSH 的正常使用。',
      'tower.strictProxyAuthMore': '查看功能说明',
      'tower.strictProxyAuthLess': '收起功能说明',
      'tower.strictProxyAuthHint': '例如实验室多人共用一台电脑时，你不希望下一位用户直接借用已经建立的校园隧道，就可以开启这个功能；个人电脑通常保持关闭即可。开启前请先点击“复制 Clash 节点”更新一次 Clash，SSH 复制一次配置即可长期使用；开启后应用内浏览器会自动认证。开关会立即保存，已连接时自动重连；重连或重启不会更换凭据，端口始终只监听 127.0.0.1。',
    ...
    en: {
      ...
      'tower.strictProxyAuthSummary': 'Leave this off by default. Campus Browser, Clash, and SSH continue to work normally.',
      'tower.strictProxyAuthMore': 'What does this do?',
      'tower.strictProxyAuthLess': 'Hide explanation',
      'tower.strictProxyAuthHint': 'Example: on a shared lab computer, enable this if you do not want the next local user to borrow an already connected campus tunnel. A personal computer can normally leave it off. Before enabling it, click “Copy Clash Node” once to update Clash and copy the SSH config once for long-term use; the built-in browser authenticates automatically. The switch saves immediately and reconnects automatically when connected. Reconnecting or restarting does not rotate the credential, and the port always stays on 127.0.0.1.',
