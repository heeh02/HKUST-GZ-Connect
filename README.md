...
已经认证的校园隧道。应用内校园浏览器会自动处理认证；Clash 和 SSH 请使用控制塔
生成的配置。只有确需兼容不支持认证的旧 SOCKS5 客户端时，才显式关闭该开关。
...
Control Tower for Clash and SSH. Turn the switch off only for a legacy SOCKS5
